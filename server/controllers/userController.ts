...
// controller function to creaet new project 
...
export const createUserProject = async (req: Request, res: Response) => {
   const userId = req.userId;
    try{
        ...
        const prompt = typeof initial_prompt === 'string' ? initial_prompt.trim() : '';
        if(!prompt){
       ...
       await prisma.user.update({
            ...
            messages: [
                {
                    ...
                    3.Describing the user expereince and interactions
        ...
        const enhancedPrompt = promptEnhanceResponse.choices[0]?.message?.content?.trim() || prompt;
...
//  controller finction to get a single user project 
