...
const Pricing = () => {
  ...
  const handlePurchase = async (planId: string) => {
        ...
        try {
          if(!session?.user)
            return toast('Please log in to make a purchase.')
          const {data} = await api.post('/api/users/purchase-credits', {planId})
          window.location.assign(data.payment_link);
          
          
        } catch (error) {
          const err = error as AxiosError<{ message?: string }>;
          toast.error(err.response?.data?.message || 'An error occurred while processing your purchase.');
          console.log(error);
